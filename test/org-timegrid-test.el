...
(ert-deftest org-timegrid-test-center-now-clamps-to-calendar-edges ()
  (with-temp-buffer
    (let ((org-timegrid--image-height 1500)
          scrolled)
      (cl-letf (((symbol-function 'decode-time)
                 (lambda (&optional _time)
                   (make-decoded-time :hour 17 :minute 0)))
                ((symbol-function 'org-timegrid--grid-top-inset)
                 (lambda () 0))
                ((symbol-function 'org-timegrid--pixels-per-minute)
                 (lambda () 1))
                ((symbol-function 'window-body-height)
                 (lambda (_window &optional _pixelwise) 600))
        ...
        (cl-letf (((symbol-function 'decode-time)
                   (lambda (&optional _time)
                     (make-decoded-time :hour 23 :minute 30))))
          (org-timegrid--center-now 'calendar-window)
          (should (= scrolled 900)))))))
